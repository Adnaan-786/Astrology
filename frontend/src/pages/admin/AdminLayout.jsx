...
export const AdminLayout = () => {
  ...
  // Guard: allow if admin_auth in localStorage OR astrovedic_user is admin
  // OR email is the fixed-admin email. Otherwise → /admin/login.
  useEffect(() => {
    let resolved = null;
    try {
      ...
      if (adminAuth) {
        resolved = adminAuth;
      } else if (userAuth && (userAuth.role === "admin" || (userAuth.email || "").toLowerCase() === fixedEmail)) {
    ...
    setAdmin({
      ...
      role: resolved.role || "SUPER_ADMIN",
      email: resolved.email || "admin123@gmail.com",
    });
  }, [navigate]);
        ...
        <div className="bg-slate-900 rounded-2xl p-8 border border-slate-800">
          ...
          <div className="mt-6 p-4 bg-amber-500/10 border border-amber-500/30 rounded-lg">
            <div className="flex items-start gap-3">
              ...
              <div>
                <p className="text-sm font-medium text-amber-400">Default Admin Credentials</p>
                <p className="text-xs text-amber-400/80 mt-1">
                  Email: <code className="bg-slate-800 px-1 rounded">admin123@gmail.com</code>
                </p>
                <p className="text-xs text-amber-400/80">
                  Password: <code className="bg-slate-800 px-1 rounded">admin@12356</code>
                </p>
                <p className="text-xs text-slate-500 mt-2">Change these after first login.</p>
              </div>
            </div>
          </div>
        </div>
