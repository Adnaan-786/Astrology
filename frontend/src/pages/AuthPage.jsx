...
const AuthPage = ({ mode = "login" }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    // Call backend so the fixed-admin email is recognised server-side.
    let userData = null;
    try {
      ...
      // Fallback: simulated client-side user (existing behaviour)
      userData = {
        id: `user_${Date.now()}`,
        name: name.trim() || emailLc.split("@")[0],
        email: emailLc,
        role: "user",
        plan: "free",
        wallet_balance: 0,
    ...
    // Hard rule: this specific email is ALWAYS admin (even if backend is down).
    if (emailLc === "akshatsharma7730@gmail.com") {
      userData.role = "admin";
    }

    try { localStorage.setItem("astrovedic_user", JSON.stringify(userData)); } catch (_) {}
    if (userData.role === "admin" || emailLc === "akshatsharma7730@gmail.com") {
      try { localStorage.setItem("admin_auth", JSON.stringify(userData)); } catch (_) {}
    }
        ...
        <div className="w-full max-w-[420px]">
          ...
          <div className="mt-5">
            <Button type="button" variant="outline" disabled
              className="w-full h-12 av-surface av-card-border opacity-60 cursor-not-allowed">
              <Mail className="mr-2 w-4 h-4" /> Continue with Google (coming soon)
            </Button>
          </div>
